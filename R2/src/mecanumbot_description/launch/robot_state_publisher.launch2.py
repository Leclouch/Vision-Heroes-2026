...
def process_ros2_controllers_config(context):
    ...
    pkg_share_description = FindPackageShare("yahboom_rosmaster_description").find(
        "yahboom_rosmaster_description"
    )

    # Define config paths using package share
    src_config_path = os.path.join(pkg_share_description, "config", robot_name)
    install_config_path = src_config_path
...
ARGUMENTS = [
    DeclareLaunchArgument(
        "robot_name", default_value="rosmaster_x3", description="Name of the robot"
    ),
    DeclareLaunchArgument(
